...
import { Mail, Phone, MapPin, Building2, Calendar, Eye } from "lucide-react";
      ...
      <div className="lg:col-span-1">
        <div className="flex gap-2 mb-4">
          {[
            ...
            { key: "all" as const, label: `All (${leads.length})` },
          ].map((f) => (
            <button
              key={f.key}
              onClick={() => setFilter(f.key)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                filter === f.key ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white"
              }`}
            >
              {f.label}
            </button>
        ...
        <div className="space-y-2">
          {displayed.length === 0 ? (
            <div className="bg-slate-800 rounded-2xl p-8 text-center text-slate-500">
              {filter === "unread" ? "All leads have been read" : "No leads yet"}
            </div>
          ) : (
            displayed.map((lead) => (
              <button
                ...
                className={`w-full text-left bg-slate-800 hover:bg-slate-700 border rounded-2xl p-4 transition-all ${
                  selected?.id === lead.id ? "border-indigo-500" : lead.isRead ? "border-slate-700" : "border-indigo-500/30"
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {lead.restaurantName && (
                      <p className="text-xs text-slate-400 truncate">{lead.restaurantName}</p>
                    )}
          ...
          <div className="bg-slate-800 rounded-2xl border border-slate-700 p-7">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                {selected.restaurantName && (
                  <p className="text-slate-400 text-sm">{selected.restaurantName}</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {selected.isRead && (
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {[
                ...
                { icon: MapPin, label: "City", val: selected.city || "—" },
                { icon: Calendar, label: "Date", val: new Date(selected.createdAt).toLocaleString() },
              ].map((row) => (
